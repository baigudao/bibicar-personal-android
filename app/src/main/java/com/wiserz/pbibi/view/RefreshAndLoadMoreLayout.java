...
/**
 * Created by jackie on 2017/8/12 11:46.
 * QQ : 971060378
 * Used as : 自定义布局，实现下拉刷新和上拉加载更多
 */
public class RefreshAndLoadMoreLayout extends ViewGroup {
    ...
    public enum State {
        RESET, PULL, REFRESHING, COMPLETE//重置,下拉状态,刷新中,完成状态
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int contentViewHeight = getMeasuredHeight() - getPaddingTop() - getPaddingBottom();
        ...
        if (!hasMeasureHeaderView) {
            ...
            //获取测量的高度
            headerViewHeight = refreshHeaderView.getMeasuredHeight();
            totalDragDistance = headerViewHeight;
        }
    ...
    public interface OnRefreshAndLoadMoreListener extends OnRefreshListener, LoadMoreListener {
    ...
    private void hideLoadView() {
        ...
        if (mLoadMoreView != null && isRecyclerView) {
            ValueAnimator animator = ValueAnimator.ofInt(0, mLoadMoreView.getMeasuredHeight());
            animator.setTarget(mLoadMoreView);
            animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
    ...
    public void setLoadMoreView(View loadMoreView) {
        ...
        ((ILoadMoreView) mLoadMoreView).getCanClickFailView().setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (isLoadingFail && onRefreshAndLoadMoreListener != null) {
                    isLoading = true;
                    ((ILoadMoreView) mLoadMoreView).loading();
                    onRefreshAndLoadMoreListener.onLoadMore();
                }
    ...
    @Deprecated
    public void loadMoreComplete(RefreshAndLoadMoreLayout.Event event) {
        if (loadMoreModel == LoadModel.ADVANCE_MODEL) {
            throw new RuntimeException("enableAdance Model cant not called closeLoadView method");
        } else {
    ...
    @Deprecated
    public void loadMoreComplete(final RefreshAndLoadMoreLayout.Event event, long delayedTime) {
        if (loadMoreModel == LoadModel.ADVANCE_MODEL) {
            throw new RuntimeException("enableAdance Model cant not called closeLoadView method");
        } else {
    ...
    private View getDefaultLoadMoreView() {
        return new SimpleLoadMoreFooterView(getContext());
    }
